...
class PetControllerAdditionalTests {
	...
	@Test
	void testProcessUpdateFormWithDuplicateNameDifferentPet() throws Exception {
		Owner owner = new Owner();
		PetType cat = new PetType();
		cat.setId(3);
		cat.setName("hamster");
		...
		owner.addPet(pet2);
		pet2.setId(2);
		given(this.owners.findById(2)).willReturn(Optional.of(owner));

		mockMvc
			.perform(post("/owners/{ownerId}/pets/{petId}/edit", 2, 2).param("name", "Buddy")
				.param("type", "hamster")
				.param("birthDate", "2015-02-12"))
	...
	void testFindOwnerNotFound() throws Exception {
		...
		assertThrows(Exception.class, () -> mockMvc.perform(get("/owners/{ownerId}/pets/new", 999)));
	}
